...
case class ArlCpgGenerator(config: FrontendConfig, rootPath: Path) extends CpgGenerator {
  private lazy val command: Path =
    if (isWin) rootPath.resolve("arl2cpg.bat") else rootPath.resolve("arl2cpg")
